...
const RegistrationPage = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (userData.password !== userData.confirmPassword) return toast.error("Passwords do not match");
    // makes sure user checks the policy box
    if (!userData.terms) return toast.error("Accept terms and policy to continue");
    ...
    <div className="flex items-center w-full h-screen bg-gradient-to-r from-black to-purple-900">
      <div className="bg-white p-6 w-300 sm:w-400 mx-auto">
        ...
        <form className="relative flex flex-col" onSubmit={handleSubmit}>
          ...
          />
          <label className="mb-5"> <input type="checkbox" onChange={getValue} checked={userData.terms} name="terms" className="mr-1" />  accept terms and Policies</label>
          <button className="bg-purple-800 py-4 hover:bg-purple-400 rounded-md">
            {" "}
            Create account{" "}
          </button>
        </form>
        <p className="mx-auto text-center mt-3">Already have an account? <Link to="/login" className="text-purple-700 hover:underline hover:opacity-90"> sign In</Link> </p>
      </div>
    </div>
  );
};
